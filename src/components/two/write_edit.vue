<template>
  <div>
    <div class="user_edit_title">
      ...
      <div class="user_edit_title">
          <p>发帖标题</p>
          <el-input v-model="title" placeholder="请输入标题">{{title}}</el-input>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted:function() {
      ...
      this.$axios({
        method:'post',
        url:'http://zhiyun_server.lilongjin.cn/api/content/edit',
        ...
        if(res.data.length != 0 ){
          //设置当前文章原分类
          this.radio = res.data.num.toString();
    ...
    methods: {
      sub(){
        if(this.radio == ""){
          ...
          this.$axios({
            method:'post',
            url:'http://zhiyun_server.lilongjin.cn/api/content/edit_save',
            data:this.qs.stringify({
              contentid: this.contentid,
