<template>
  <div>
    <div class="article" v-for="item in question">
    ...
    created: function () {
      ...
      this.$axios({
        ...
        for(var i = 0; i<content_alls.length; i++){
          if(content_alls[i].category.name == "论坛问答"){
            this.question.push(content_alls[i]);
          }
        };
      }).catch((error) => {
        console.log(error);
        ...
        this.$axios({
          ...
          if(res.data != ""){
            ...
            for(var i = 0; i<new_content.length; i++){
              if(new_content[i].category.name == "论坛问答"){
                this.question.push(new_content[i]);
              }
            };
          }else{
            this.$message({
